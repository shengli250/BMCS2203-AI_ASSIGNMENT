...
def load_resources():
    ...
    try:
        ...
        lr_model = joblib.load('logistic_regression_intent_model.joblib')
        ...
        st.error(f"Error loading required model files. Please ensure all files are in the same directory. Missing file: {e.filename}")
        return None, None

lr_model, vectorizer = load_resources()

# --- D. Prediction Function ---
def predict_intent(text):
    """
    Predicts the intent using the Logistic Regression model and applies a confidence threshold.
    """
    if lr_model is None or vectorizer is None or not is_nltk_ready:
    ...
    # LogisticRegression provides probabilities via predict_proba
    predictions_proba = lr_model.predict_proba(vector)[0]
    ...
    # Get the predicted intent name (LogisticRegression.classes_ contains the intent names)
    predicted_intent_name = lr_model.classes_[predicted_index]
...
def main():
    st.set_page_config(page_title="LR Intent Chatbot (Chat History)", layout="centered")

    st.title("🛎️ Hotel Chatbot (Logistic Regression)")
